...
async function cargarTablaHorarioModificar(idHorarioEncb) {
    ...
    try {
        ...
        // Cargar las fechas
        document.getElementById('fecha_inicio_actualizar').value = data.fecha_inicio;
        document.getElementById('fecha_fin_actualizar').value = data.fecha_fin;

        // Generar las filas de la tabla
        const tbody = document.getElementById('tabla_horarios_actualizar').querySelector('tbody');
...
async function actualizarHorario() {
    try {
        ...
        // Obtener las fechas
        const fechaInicio = document.getElementById('fecha_inicio_actualizar').value;
        ...
        const detalles = Array.from(filas).map(fila => {
            ...
            const idEmpleado = fila.cells[1].querySelector('input[name="id_empleado"]').value;
...
document.getElementById('btnCancelarAgregarHorario').addEventListener('click', function () {
    document.getElementById('vista_horarios').style.display = 'block';
    document.getElementById('vista_nuevo_horario').style.display = 'none';
});

document.getElementById('btnCancelarActualizarHorario').addEventListener('click', () => {
